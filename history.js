...
function renderMovements() {
    ...
    filtered.forEach(m => {
        ...
        if (m.movementType === 'spend') {
            extra = `المقاول: ${m.contractor || '-'}`;
            if (m.remainingWithContractor !== null && m.remainingWithContractor !== undefined) {
                extra += ` · متبقي بالعربية: ${m.remainingWithContractor}`;
            }
        }
...
function openEdit(id) {
    ...
    if (m.movementType === 'spend') {
        fieldsHtml += `
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">المقاول/العمالة</label>
                <input type="text" id="editContractor" value="${m.contractor || ''}" class="input-field block w-full p-2">
            </div>`;
        if (m.remainingWithContractor !== null && m.remainingWithContractor !== undefined) {
            fieldsHtml += `
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">متبقي في العربية</label>
                <input type="number" id="editRemaining" value="${m.remainingWithContractor ?? ''}" step="any" class="input-field block w-full p-2">
            </div>`;
        }
    }
